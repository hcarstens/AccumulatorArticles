...
Growing demand therefore pushes price upward. The US dollar faces no such cap; more dollars can be issued whenever policymakers choose.

## 1 Supply, Demand, and the Standard Model
...
## 2 Supply, Demand and the Special Case of Fixed Supply
...
* **Fixed-supply scenario.** Bitcoin's total supply is capped at 21 million coins.
  ...
  because \(Q_s\) cannot rise. The US dollar is the opposite: supply expands whenever more dollars are printed.
...
With limitless production capacity, suppliers meet any demand at the going price. Even with the same
2 % demand drift, the equilibrium price stays flat.

```{python}
#| label: fig-price-path-infinite
#| fig-cap: "Price path with infinite supply and 2 % demand growth"
#| fig-alt: "Flat price line at $10 over 24 periods"
#| fig-width: 6
#| fig-height: 3.5
import numpy as np
import matplotlib.pyplot as plt
...
P0 = 10
t = np.arange(0, 25)
prices = np.full_like(t, P0, dtype=float)

plt.plot(t, prices, label="Equilibrium price")
plt.title("Price path with infinite supply")
plt.xlabel("Period")
plt.ylabel("Price")
plt.legend()
plt.tight_layout()
```
Under unlimited supply, price does not rise; it remains anchored at $10 despite demand growth.

## Conclusion

Bitcoin's fixed supply leads to accelerating price appreciation, while the infinitely expandable US dollar supply keeps prices stable.
